/*
  js/cart.js
  - Simple shopping cart helpers that store the cart in localStorage under the key 'minishop_cart'.
  - This file is intentionally commented with educational notes so learners understand what each
    function does and why it is implemented this way.

  Data format (stored in localStorage):
    minishop_cart = [ { name: string, price: number }, { name: string, price: number }, ... ]

  Notes for integration:
  - Each product card in index.html contains a hidden element with class `product-item` and
    data attributes data-name and data-price. The function addToCartFromCard(button) expects the
    button to be inside the same card-body so it can find the .product-item and read those values.
  - cart.html includes code to render the cart contents and calls updateBadge() after mutations.
*/

// Load cart from localStorage (if missing, default to an empty array).
// Using JSON.parse + || [] ensures old or empty values are handled safely.
